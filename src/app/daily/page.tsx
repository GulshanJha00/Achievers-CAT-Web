...
import { ArrowRight, BookOpenCheck, Brain, CheckCircle2, Flame, Loader2, Network, Users } from "lucide-react";
...
export default function DailyPage() {
  ...
  // Each leaderboard has one document per student, so this is an accurate
  // live people-attempted count and does not count resubmissions twice.
  useEffect(() => {
    const unsubscribers = (["quant", "varc", "dilr"] as Section[]).map((section) =>
      onSnapshot(
        collection(db, "daily_leaderboards", `${date}_${section}`, "entries"),
        (snapshot) => setStats((previous) => ({ ...previous, [section]: snapshot.size })),
  ...
  const items = [
    { key: "quant" as Section, title: "Quantitative Aptitude", desc: `${data?.quant?.length || 5} questions`, icon: Brain },
    { key: "varc" as Section, title: data?.varc?.type === "VA" ? "VA of the Day" : "RC of the Day", desc: data?.varc?.type === "VA" ? `${data?.varc?.questions?.length || 5} VA questions` : `${data?.varc?.questions?.length || 5} questions`, icon: BookOpenCheck },
    { key: "dilr" as Section, title: "DILR Set of the Day", desc: `${data?.dilr?.questions?.length || 5} questions`, icon: Network },
  ];
    ...
    {!data || data.published === false ? <div className="mt-8 rounded-2xl border border-border bg-white p-8 text-center"><h2 className="font-display text-xl font-bold">Today&apos;s practice is being prepared.</h2><p className="mt-2 text-sm text-muted">Please check back shortly.</p></div> : <div className="mt-8 grid gap-4">
      {items.map((item) => {
        ...
        return <div key={item.key} className="rounded-2xl border border-border bg-white p-5 transition hover:border-brand/40 hover:shadow-sm">
          <div className="flex items-center justify-between gap-4"><div className="flex items-center gap-4"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-brand-tint text-brand-darker"><Icon size={20}/></div><div><h2 className="font-display text-lg font-bold">{item.title}</h2><p className="mt-1 text-sm text-muted">{item.desc} · <span className="font-semibold text-brand-darker">15 minutes</span></p></div></div><div className="hidden items-center gap-1.5 text-xs font-semibold text-muted sm:flex"><Users size={14}/> {stats[item.key]} attempted</div></div>
