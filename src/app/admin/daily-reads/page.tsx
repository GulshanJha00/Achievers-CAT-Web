...
type DailyRead = { id: string; title: string; url: string; kind: "pdf" | "link"; published?: boolean; publishedFor?: string };
...
function DailyReadsManager() {
  ...
  const [publishedFor, setPublishedFor] = useState(todayIST());
  ...
  function resetForm() {
    setTitle(""); setUrl(""); setFile(null); setEditing(null); setPublishedFor(todayIST()); setMessage("");
    if (fileInput.current) fileInput.current.value = "";
  }

  function startEditing(read: DailyRead) {
    setEditing(read); setTitle(read.title); setUrl(read.url); setFile(null); setPublishedFor(read.publishedFor || todayIST()); setMessage("");
    if (fileInput.current) fileInput.current.value = "";
  }
  ...
  async function saveRead(event: React.FormEvent) {
    ...
    try {
      ...
      const fields = { title: title.trim(), url: readUrl, kind: file ? "pdf" : "link", published: true, publishedFor, updatedAt: serverTimestamp() };
      if (editing) {
  ...
  return <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 lg:px-8">
    ...
    <p className="mt-2 text-sm text-muted">Publish a newspaper PDF or an essay/article link for signed-in students. Reads automatically disappear after their selected IST date.</p>
    <form onSubmit={saveRead} className="mt-8 space-y-5 rounded-2xl border border-border bg-white p-6">
      ...
      <label className="block text-sm font-semibold">Show on date (IST)<input type="date" value={publishedFor} onChange={(event) => setPublishedFor(event.target.value)} required className="mt-2 w-full rounded-xl border border-border px-3 py-2.5 font-normal outline-none focus:border-brand" /></label>
