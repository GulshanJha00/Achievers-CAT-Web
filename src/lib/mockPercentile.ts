...
const reference = [[0, 0], [25, 60], [40, 80], [52, 90], [63, 95], [84, 99], [95, 99.5], [113, 99.9]] as const;
...
function benchmarkPercentile(score: number, total: number, difficulty: Difficulty) {
  const scaledScore = Math.max(0, score) * (198 / Math.max(1, total * 3)) / (difficultyMultiplier[difficulty || "CAT Level"] || 1);
